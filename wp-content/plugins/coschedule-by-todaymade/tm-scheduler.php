...
if ( ! class_exists( 'tm_coschedule' ) ) {
    ...
    class TM_CoSchedule {
        ...
        public function add_menu() {
            add_menu_page( 'CoSchedule Calendar', 'Calendar', 'edit_posts', 'tm_coschedule_calendar', array( $this, 'plugin_calendar_page' ),
        ...
        public function add_submenu() {
            if ( true === $this->connected ) {
                ...
                add_submenu_page( 'tm_coschedule_calendar', 'Analytics', 'Analytics', 'edit_posts', 'tm_coschedule_top_posts', array( $this, 'plugin_analytics_page' ) );
                ...
                add_submenu_page( 'tm_coschedule_calendar', 'Help', 'Help', 'edit_posts', 'tm_coschedule_help', array( $this, 'plugin_help_page' ) );
            }
        }
        ...
        public function plugin_analytics_page() {
            ...
            $redirect = 'stats.analytics.dashboard';
            include( sprintf( "%s/frame.php", dirname( __FILE__ ) ) );
        }
        ...
        public function fix_protocol_relative_url( $url ) {
            // If the URL starts with '//', get the protocol from $site_url,
            ...
            if ( substr( $url, 0, 2 ) === '//' ) {
                ...
                if ( function_exists( 'wp_parse_url' ) ) {
                    ...
                    if ( $parsed_url !== false && is_array( $parsed_url ) ) {
                        $protocol = $parsed_url['scheme'];
                    }
                }

                // If 'scheme' (i.e. protocol) wasn't found, default to 'http'.
        ...
        public function delete_category_callback( $category_id ) {
            if ( true === $this->connected ) {
                $this->api_post( '/hook/wordpress_categories/delete?_wordpress_key=' . $this->token, array( 'cat_id' => $category_id ) );
            }
        }
